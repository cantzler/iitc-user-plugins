...
//
// Unless you understand that it is an experimental variation of the player tracker plugin
// This is NOT the official plugin, the name and version have been left the same to make
// it's replacement by the official script easier when testing.
// This script has been modified to track the MU gains and losses per player
// as with the original script no additional server calls are made and only downloaded
// comm data is used in looking for field information
// Drawbacks : because of the lack of unique id's it cannot track when two fields of the same mu
// are made at the same time, though that's less likely on fields of larger mu 
// ToDo : This may be better as a plugin to augment the standard player tracker
// Yes, my code formatting isn't what you normally see in javascript ;-P

// Changes are indicated by  (Ray)  in the comments 
// Raymond Andrews  @SteelToad  randrews.mobile@gmail.com

// 2015-04-09-2045  Fixed: Was using wrong variable name causing plugin not to calculate fade amount for older markers


// ==UserScript==
// @id             iitc-plugin-mu-tracker
// @name           IITC Plugin: MU tracker
// @category       Layer
...
// plugin_info.buildName = 'steeltoad';
// plugin_info.dateTimeVersion = '20150409.2045';
// plugin_info.pluginId = 'mu-tracker';
...
window.MU_TRACKER_MIN_OPACITY = 0.3;
window.MU_TRACKER_LINE_COLOUR = '#FF00FD';


// use own namespace for plugin
window.plugin.muTracker = function() {};

window.plugin.muTracker.setup = function() {
  $('<style>').prop('type', 'text/css').html('.plugin-mu-tracker-popup a {\n	color: inherit;\n	text-decoration: underline;\n	text-decoration-style: dashed;\n	-moz-text-decoration-style: dashed;\n	-webkit-text-decoration-style: dashed;\n}\n').appendTo('head');
  ...
  plugin.muTracker.iconEnl = L.Icon.Default.extend({options: {
    iconUrl: iconEnlImage,
    iconRetinaUrl: iconEnlRetImage
  }});
  plugin.muTracker.iconRes = L.Icon.Default.extend({options: {
    iconUrl: iconResImage,
    iconRetinaUrl: iconResRetImage
  }});

  plugin.muTracker.drawnTracesEnl = new L.LayerGroup();
  plugin.muTracker.drawnTracesRes = new L.LayerGroup();
  // to avoid any favouritism, we'll put the player's own faction layer first
  if (PLAYER.team == 'RESISTANCE') {
    window.addLayerGroup('MU Tracker Resistance', plugin.muTracker.drawnTracesRes, true);
    window.addLayerGroup('MU Tracker Enlightened', plugin.muTracker.drawnTracesEnl, true);
  } else {
    window.addLayerGroup('MU Tracker Enlightened', plugin.muTracker.drawnTracesEnl, true);
    window.addLayerGroup('MUr Tracker Resistance', plugin.muTracker.drawnTracesRes, true);
  }
  map.on('layeradd',function(obj) {
    if(obj.layer === plugin.muTracker.drawnTracesEnl || obj.layer === plugin.muTracker.drawnTracesRes) {
  ...
  plugin.muTracker.playerPopup = new L.Popup({offset: L.point([1,-34])});

  addHook('publicChatDataAvailable', window.plugin.muTracker.handleData);

  window.map.on('zoomend', function() {
    window.plugin.muTracker.zoomListener();
  });
  window.plugin.muTracker.zoomListener();
  
  plugin.muTracker.setupUserSearch();
}

window.plugin.muTracker.stored = {};

plugin.muTracker.onClickListener = function(event) {
  var marker = event.target;

  if (marker.options.desc) {
    plugin.muTracker.playerPopup.setContent(marker.options.desc);
    plugin.muTracker.playerPopup.setLatLng(marker.getLatLng());
    map.openPopup(plugin.muTracker.playerPopup);
  }
};

// force close all open tooltips before markers are cleared
window.plugin.muTracker.closeIconTooltips = function() {
  plugin.muTracker.drawnTracesRes.eachLayer(function(layer) {
    if ($(layer._icon)) { $(layer._icon).tooltip('close');}
  });
  plugin.muTracker.drawnTracesEnl.eachLayer(function(layer) {
    if ($(layer._icon)) { $(layer._icon).tooltip('close');}
  });
}

window.plugin.muTracker.zoomListener = function() {
  var ctrl = $('.leaflet-control-layers-selector + span:contains("MU Tracker")').parent();
  if(window.map.getZoom() < window.MU_TRACKER_MIN_ZOOM) {
    if (!window.isTouchDevice()) plugin.muTracker.closeIconTooltips();
    plugin.muTracker.drawnTracesEnl.clearLayers();
    plugin.muTracker.drawnTracesRes.clearLayers();
    ctrl.addClass('disabled').attr('title', 'Zoom in to show those.');
    //note: zoomListener is also called at init time to set up things, so we only need to do this in here
    window.chat.backgroundChannelData('plugin.muTracker', 'all', false);   //disable this plugin's interest in 'all' COMM
  } else {
    ctrl.removeClass('disabled').attr('title', '');
    //note: zoomListener is also called at init time to set up things, so we only need to do this in here
    window.chat.backgroundChannelData('plugin.muTracker', 'all', true);    //enable this plugin's interest in 'all' COMM
  }
}

window.plugin.muTracker.getEventLimit = function() // (Ray) renamed to use renamed variable
    {
    return new Date().getTime() - window.MU_TRACKER_MAX_EVENT_TIME;
    }

window.plugin.muTracker.getFieldLimit = function() // (Ray) separate limit for fields
    {
    return new Date().getTime() - window.MU_TRACKER_MAX_FIELD_TIME;
    }

window.plugin.muTracker.discardOldData = function() {
    // (Ray) altered to remove old fields as well as old events
    var eventlimit = plugin.muTracker.getEventLimit();  // (Ray)
    var fieldlimit = plugin.muTracker.getFieldLimit();  // (Ray)
        
    $.each(plugin.muTracker.stored, function(plrname, player) {
        var i;
        ...
        if (hasEvents)
            ...
            if (i > 0) 
                ...
                if (i === ev.length) 
                    ...
                    plugin.muTracker.stored[plrname].events.splice(0, i); 
                }
            }
        ...
        if (hasFields)
            ...
            if (i > 0)
                ...
                if (i === fld.length) 
                    ...
                    plugin.muTracker.stored[plrname].fields.splice(0, i); 
                }
            }
        
        if (!hasEvents && !hasFields)
            delete plugin.muTracker.stored[plrname];
    });
}

window.plugin.muTracker.eventHasLatLng = function(ev, lat, lng) {
  var hasLatLng = false;
...
window.plugin.muTracker.processNewData = function(data) 
    {
    ...
    var eventlimit = plugin.muTracker.getEventLimit();
    var fieldlimit = plugin.muTracker.getFieldLimit();
    ...
    $.each(data.result, function(ind, json) 
        ...
        var playerData = window.plugin.muTracker.stored[plrname];
        if(!playerData) 
            {
            plugin.muTracker.stored[plrname] = 
                {
                nick: plrname,
            ...
            playerData = window.plugin.muTracker.stored[plrname];
            }
        ...
        if (isEvent)
            ...
            if (playerData.events.length === 0)
                {
                plugin.muTracker.stored[plrname].events.push(newEvent);
                }
            else
                {
                ...
                if(evts[cmp].time === json[1]) 
                    ...
                    plugin.muTracker.stored[plrname].events = evts;
                ...
                if (evts[cmp+1] && plugin.muTracker.eventHasLatLng(evts[cmp+1], lat, lng))
                    handledThisEvent = true;

                if (!handledThisEvent)
                    {
                    // if this event is newer, need to look at the previous one
                    var sameLocation = plugin.muTracker.eventHasLatLng(evts[cmp], lat, lng);
        ...
        if (isField)
            ...
            if (playerData.fields.length === 0)
                {
                plugin.muTracker.stored[plrname].fields.push(newField);
                }
            else
                {
                ...
                // update player data
                plugin.muTracker.stored[plrname].fields = flds;
                }
            } //end isField
...
window.plugin.muTracker.getLatLngFromEvent = function(ev) {
...
window.plugin.muTracker.ago = function(time, now) {
  var s = (now-time) / 1000;
...
window.plugin.muTracker.drawData = function() {
  var isTouchDev = window.isTouchDevice();

  var gllfe = plugin.muTracker.getLatLngFromEvent;

  var polyLineByAgeEnl = [[], [], [], []];
  var polyLineByAgeRes = [[], [], [], []];

  var split = MU_TRACKER_MAX_EVENT_TIME / 4;
  var now = new Date().getTime();
    
  $.each(plugin.muTracker.stored, function(plrname, playerData) {
    ...
    //-----------------  
    var popup = $('<div>')
      .addClass('plugin-mu-tracker-popup');
    ...
    popup
      ...
      .append('<br>')
      .append(plugin.muTracker.getPortalLink(last))
      .append('<br>');
    ...
    if(evtsLength >= 2) 
        ...
        for(var i = evtsLength - 2; i >= 0 && i >= evtsLength - window.MU_TRACKER_MAX_EVENT_DISPLAY; i--) 
            {
            var ev = playerData.events[i];
            $('<tr>')
              .append($('<td>') .text(ago(ev.time, now) + ' ago '))
              .append($('<td>') .append(plugin.muTracker.getPortalLink(ev)))
            .appendTo(eventtable);
            }
    ...
    if (playerData.fields.length > 0)
        ...
        for(var i = playerData.fields.length-1; i >= 0; i--)
            {
            if ((playerData.fields[i].mutype == '+') && (plusLines < window.MU_TRACKER_MAX_FIELD_DISPLAY))
                { 
                ++plusFields; 
                plusMU += playerData.fields[i].mu; 
                $('<tr>')
                  .append($('<td>') .text(ago(playerData.fields[i].time, now) + ' ago'))
                  .append($('<td>') .append(plugin.muTracker.getPortalLink(playerData.fields[i])))
                  .appendTo(plustable);
                ++plusLines;
                }
            if ((playerData.fields[i].mutype == '-') && (minusLines < window.MU_TRACKER_MAX_FIELD_DISPLAY))
                { 
                ++minusFields; 
    ...
    // marker opacity
    var relOpacity = 1 - (now - last.time) / window.MU_TRACKER_MAX_EVENT_TIME // (Ray) fixed 2015-04-09-2045
    var absOpacity = window.MU_TRACKER_MIN_OPACITY + (1 - window.MU_TRACKER_MIN_OPACITY) * relOpacity;

    // marker itself
    var icon = playerData.team === 'RESISTANCE' ?  new plugin.muTracker.iconRes() :  new plugin.muTracker.iconEnl();
    ...
    m.addEventListener('spiderfiedclick', plugin.muTracker.onClickListener);
    ...
    m.addTo(playerData.team === 'RESISTANCE' ? plugin.muTracker.drawnTracesRes : plugin.muTracker.drawnTracesEnl);
  ...
  $.each(polyLineByAgeEnl, function(i, polyLine) {
    ...
    $.each(polyLine,function(ind,poly) {
      L.polyline(poly, opts).addTo(plugin.muTracker.drawnTracesEnl);
    });
  });
  $.each(polyLineByAgeRes, function(i, polyLine) {
    ...
    $.each(polyLine, function(ind,poly) {
      L.polyline(poly, opts).addTo(plugin.muTracker.drawnTracesRes);
    });
  });
}


window.plugin.muTracker.getPortalLink = function(data) {
...
window.plugin.muTracker.handleData = function(data) {
  if(window.map.getZoom() < window.MU_TRACKER_MIN_ZOOM) return;

  plugin.muTracker.discardOldData();
  plugin.muTracker.processNewData(data);
  if (!window.isTouchDevice()) plugin.muTracker.closeIconTooltips();

  plugin.muTracker.drawnTracesEnl.clearLayers();
  plugin.muTracker.drawnTracesRes.clearLayers();
  plugin.muTracker.drawData();
}

window.plugin.muTracker.findUser = function(nick) {
...
window.plugin.muTracker.findUserPosition = function(nick) {
  var data = window.plugin.muTracker.findUser(nick);
  if (!data) return false;

  var last = data.events[data.events.length - 1];
  return plugin.muTracker.getLatLngFromEvent(last);
}

window.plugin.muTracker.centerMapOnUser = function(nick) {
  var data = plugin.muTracker.findUser(nick);
  if(!data) return false;

  var last = data.events[data.events.length - 1];
  var position = plugin.muTracker.getLatLngFromEvent(last);

  if(window.isSmartphone()) window.show('map');
  window.map.setView(position, map.getZoom());

  if(data.marker) {
    window.plugin.muTracker.onClickListener({target: data.marker});
  }
  return true;
}

window.plugin.muTracker.onNicknameClicked = function(info) {
  if (info.event.ctrlKey || info.event.metaKey) {
    return !plugin.muTracker.centerMapOnUser(info.nickname);
  }
  return true; // don't interrupt hook
}

window.plugin.muTracker.onSearchResultSelected = function(result, event) {
  ...
  if(false === window.plugin.muTracker.centerMapOnUser(result.nickname))
...
window.plugin.muTracker.onSearch = function(query) {
  var term = query.term.toLowerCase();

  if (term.length && term[0] == '@') term = term.substr(1);

  $.each(plugin.muTracker.stored, function(nick, data) {
    ...
    query.addResult({
      ...
      position: plugin.muTracker.getLatLngFromEvent(event),
      onSelected: window.plugin.muTracker.onSearchResultSelected,
    });
  });
}

window.plugin.muTracker.setupUserSearch = function() {
  addHook('nicknameClicked', window.plugin.muTracker.onNicknameClicked);
  addHook('search', window.plugin.muTracker.onSearch);
}


var setup = plugin.muTracker.setup;
